#ifndef INDEX_TAUMNG_H
#define INDEX_TAUMNG_H
...
class IndexTauMNG : public Index {
   public:
    ...
    void Search_QEO(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<int> >& perm_list, int gtNN, float* trans_q, float* trans_data);
    void Search_QEO_PDP(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<int> >& perm_list, int gtNN, float* trans_q, float* trans_data);
    void Search_QEO_PDP_PII(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<float> >& uT2, float* trans_q, float* trans_data, float* data_step_sum, std::vector<float>& trans_q_steps);
   ...
   public:
    ...
    double NDC;
    double not_break_comp_NDC;
    double not_break_comp_amount;
    int hops;
    float ang;
    float avg_tau;
    double comp_amount;
