...
IndexTauMNG::IndexTauMNG(const size_t dimension, const size_t n, Metric m, Index* initializer)
...
void IndexTauMNG::get_neighbors(const float* query, const Parameters& parameter, std::vector<Neighbor>& retset, std::vector<Neighbor>& fullset) {
...
void IndexTauMNG::get_neighbors(const float* query, const Parameters& parameter, boost::dynamic_bitset<>& flags, std::vector<Neighbor>& retset, std::vector<Neighbor>& fullset) {
    unsigned L = parameter.Get<unsigned>("L");
...
void IndexTauMNG::InterInsert(unsigned n, unsigned range, std::vector<std::mutex>& locks, SimpleNeighbor* cut_graph_) {
...
void IndexTauMNG::Search(const float* query, const float* x, size_t k, const Parameters& parameters, unsigned* indices) {};

void IndexTauMNG::Search_QEO(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<int> >& perm_list, int gtNN, float* trans_q, float* trans_data) {
    ...
    while (k < (int)stopL) {
        ...
        if (retset[k].flag) {
            ...
            if (k < kperc * L) {
                ...
                float firstShortestDist = 1000000.0;
                int id_firstShortest = -1;
                float secondShortestDist = 2000000.0;
                int id_secondShortest = -1;
...
void IndexTauMNG::Search_QEO_PDP(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<int> >& perm_list, int gtNN, float* trans_q, float* trans_data) {
    const unsigned L = parameters.Get<unsigned>("L_search");
    unsigned stopL = parameters.Get<unsigned>("stopL");
    if (stopL > L) {
        stopL = L;
...
void IndexTauMNG::Search_QEO_PDP_PII(const float* query, const float* x, size_t K, const Parameters& parameters, unsigned* indices, std::vector<std::vector<float> >& uT2, float* trans_q, float* trans_data, float* data_step_sum, std::vector<float>& trans_q_steps) {
    const unsigned L = parameters.Get<unsigned>("L_search");
    unsigned stopL = parameters.Get<unsigned>("stopL");
    if (stopL > L) {
        stopL = L;
...
void IndexTauMNG::findroot(boost::dynamic_bitset<>& flag, unsigned& root, const Parameters& parameter) {
    unsigned id = nd_;
    for (unsigned i = 0; i < nd_; i++) {
        if (flag[i] == false) {
            id = i;
...
void IndexTauMNG::DFS(boost::dynamic_bitset<>& flag, unsigned root, unsigned& cnt) {
    unsigned tmp = root;
